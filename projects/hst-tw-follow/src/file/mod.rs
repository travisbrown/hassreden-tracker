...
pub fn read_batches<P: AsRef<Path>>(base: P) -> BatchIterator {
    list_batch_files(base).map_or_else(
        |error| BatchIterator::Failed(Some(error)),
        |mut files| {
            files.reverse();
            BatchIterator::Remaining(files)
        },
    )
}
                ...
                files
                    ...
                    .map(|(timestamp, user_id, followers_path, following_path)| {
                        let follower_change = followers_path
                            .map_or_else(|| Ok(Change::default()), |path| read_batch_file(path))?;
...
fn list_batch_files<P: AsRef<Path>>(
    ...
    for entry in std::fs::read_dir(&base)? {
        ...
        let mut followers = list_timestamped_files(path.join("followers"))?;
        let mut following = list_timestamped_files(path.join("following"))?;
